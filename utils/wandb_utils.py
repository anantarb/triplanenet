...
class WBLogger:
    ...
    def __init__(self, opts):
        ...
        wandb.init(project="triplanenet", config=vars(opts), name=wandb_run_name)
    ...
    @staticmethod
    def log_images_to_wandb(x, y_hat, y_hat_psp, id_logs, prefix, step, opts):
        im_data = []
        column_names = ["Source", "Output (TriPlaneNet)", "Output (pSp)"]
        ...
        for i in range(len(x)):
            cur_im_data = [
                ...
                wandb.Image(common.tensor2im(y_hat_psp[i])),
            ]
            if id_logs is not None:
