...
def run():
    ...
    command = f"python scripts/inference.py --exp_dir {test_opts.exp_dir} --checkpoint_path {test_opts.checkpoint_path} --data_path {test_opts.data_path} --resize_outputs --novel_view_angles {angles_string}--test_batch_size {test_opts.test_batch_size} --test_workers {test_opts.test_workers}"
    if test_opts.CTTR:
        command += " --CTTR"
    os.system(command)
